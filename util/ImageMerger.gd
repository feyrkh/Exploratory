...
static func merge_images(image_merge_info:Array, noise:FastNoiseLite=null, noise_cutoff:float=1.0, tree:SceneTree=null) -> Texture2D: # Array[ImageMergeInfo] as input
	...
	for merge_info in image_merge_info:
		if base_img == null:
			...
			await modulate_image(overlay_img, merge_info.modulate, noise, noise_cutoff, tree)
...
static func modulate_image(img:Image, color:Color, noise:FastNoiseLite, noise_cutoff:float, tree:SceneTree=null):
