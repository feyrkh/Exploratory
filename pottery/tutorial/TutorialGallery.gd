...
@export var tutorial_steps := [
	"res://pottery/tutorial/GalleryUnpackItemTutorial.tscn",
	"res://pottery/tutorial/GalleryResizeItemTutorial.tscn",
	"res://pottery/tutorial/GalleryBackgroundTutorial.tscn",
]
...
func _ready():
	if Global.has_completed_gallery_tutorial:
		visible = false
		queue_free()
		return
	show_next_step()
...
func next_button_pressed():
	if cur_tutorial_idx >= tutorial_steps.size() - 1:
		queue_free()
		Global.has_completed_gallery_tutorial = true
	else:
		show_next_step()
